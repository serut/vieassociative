...
// Parsley.JS launch on every form
var confParsley = {
  ...
  listeners: {
      ...
      onFormSubmit: function ( isFormValid, event, ParsleyForm ) {
          ...
          form.find('.wysiwyg-editor').each(function(){
            textarea.push({'name':$(this).attr('data-name'), 'value' : $(this).cleanHtml()});
          })
          var data = form.serializeArray().concat(textarea);
          if(isFormValid){
              // Display a nice loading
              if(form.attr('data-loading')){
                el = form.closest('div')
                el.after('<div id="spinner-load" class="spinner-container"><div class="spinner-load"></div></div>');
                $('#spinner-load').height(el.outerHeight()).width(el.outerWidth()).css('margin-top',-(el.height()+31));
              }
              $(form).find('.error-laravel').empty();
              $.ajax({
                  ...
                  success: function (data) {
                      if(IsJsonString(data)){
                        if(data['success']){
                            ...
                            if ( !data || !data.length ) {
                                if(data.responseText)
                                  ...
                                  d = data;
                                if(d != ""){
                                  $.each(d['error-message'],function(){
                                    $(form).find('.error-laravel').append(this);
                                    console.log($(form).find('.error-laravel'));
                                  });
                                  //console.log(d);
                                }else{
                        ...
                        console.log("Cette erreur est tout à fait anormale et va demander à notre équipe d'intervenir pour résoudre ce problème au plus vite.");
                        console.log("Vous pouvez par ailleur prendre contact avec notre équipe si vous souhaitez apporter votre aide.");
                        console.log(data);
                      }
                  },
                  error: function(data){
                      $('#spinner-load').remove();
                      if(data.responseText)
                        d = JSON.parse(data.responseText).error;
                      else
                        d = data.error;
                      alert('Server error.\nWe apologize for the inconvenient.\n'+d.type+' : '+ d.message + '\non '+d.file+' line '+d.line);
                  }
              });
          }
      },
      onFieldError: function ( elem, constraints, ParsleyField ) {
          $(elem).parent().parent().find('label').removeClass('text-success').addClass('text-error');
      },
      onFieldSuccess: function ( elem, constraints, ParsleyField ) {
          $(elem).parent().parent().find('label').removeClass('text-error').addClass('text-success');
      }
  }
}
function callback(){
  $("form[data-validate=our-parsey]").attr('parsley',"true").parsley(confParsley);
  $("form[data-validate=our-parsey-1]").attr('parsley',"true").parsley(confParsley);
  $("form[data-validate=our-parsey-2]").attr('parsley',"true").parsley(confParsley);
  $("form[data-validate=our-parsey-3]").attr('parsley',"true").parsley(confParsley);
}
...
/* Parsley dist/parsley.extend.min.js build version 1.1.17-dev http://parsleyjs.org */
window.ParsleyConfig=window.ParsleyConfig||{};
